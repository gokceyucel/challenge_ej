...
// Gets a single Invite from the DB by id
...
// Gets a single Invite from the DB by token
export function showByToken(req, res) {
  return Invite.findOne({token: req.params.token}).exec()
    .then(handleEntityNotFound(res))
    .then(respondWithResult(res))
    .catch(handleError(res));
}
...
export function upsert(req, res) {
  ...
  return Invite.findOneAndUpdate({_id: req.params.id}, req.body, {
    upsert: true,
